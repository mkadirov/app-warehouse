...
public class InputService {
    ...
    public Result getInputList(){
        return new Result("Successfully retrieved", true, inputRepository.findAll());
    }
    ...
    public Result getInputById(Integer id){
        ...
        return optionalInput.map(input ->
                new Result("Successfully retrieved", true, input)).orElseGet(() ->
                new Result("Input not found", false));
    }

    //PUT method to update Input by ID
    public Result uploadInput(Integer id, InputDto inputDto){
        Optional<Input> optionalInput = inputRepository.findById(id);
